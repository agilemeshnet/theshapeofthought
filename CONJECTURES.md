...
- **Learning prediction:** If learning is phase-transition at the node, then learning curves should show punctuated equilibria at fine temporal resolution, even when they appear smooth at coarse resolution. Micro-electrode recordings during skill acquisition should show individual synapses snapping between bistable states, not sliding along gradients. (This is consistent with existing evidence on synaptic bistability - Bhatt et al. 2009 - but the fishing net model predicts that the DISTRIBUTION of snap times across the net should follow the geometry of the net's topology, not a random distribution.)

- **Sleep prediction:** If consolidation is palette-stacking, then the order of replay during sleep should follow the temporal order of encoding (most recent first, diminishing depth), and the INTERFERENCE patterns between layers should depend on the topological distance between the knots that nestle together, not just the temporal distance between the experiences. Two experiences with similar topology (similar knot patterns) should interfere more during consolidation than two experiences with different topology, regardless of temporal proximity.

- **Strange attractor prediction:** If intent reshapes probability landscapes rather than determining outcomes, then the confidence distribution at each level of a hierarchical decision system should narrow in the presence of a consistent intent signal (strange attractor present) and widen in its absence (attractor removed), with the narrowing rate proportional to the attractor's strength. Removing the Fish should measurably widen the bell curve at every floor.

- **Bandwidth prediction:** If propagation delay bounds temporal resolution, then the maximum frequency at which a distributed system can respond coherently should equal the inverse of its diameter (in propagation-delay units). Larger murmurations should have lower bandwidth. This is testable in both biological (starling flocks of different sizes) and artificial (distributed agent networks with different topologies) systems.
...
This maps directly onto the Flock tick. During each tick:
1. The jury forms (Ask-sibling fires)
2. Harsanyi dividends are computed for the coalition
3. Each cell's Shapley value (Lloyd Shapley, Nobel Prize 2012) weights its vote by genuine marginal contribution, not volume
4. Cells with negligible Shapley values are pruned from the jury without disrupting the equilibrium
...
- **Three-button sufficiency:** If Reynolds' three rules are sufficient for cooperative equilibrium in flocks, and the three buttons are structurally isomorphic to Reynolds' three rules, then a Diorama cell with only three buttons should achieve cooperative equilibrium in any repeated-interaction scenario. Removing ANY one button should break the equilibrium. Specifically: removing Dismiss should produce coercion (agents cannot maintain boundaries). Removing Ask-sibling should produce isolation (agents cannot coordinate). Removing Act should produce paralysis (agents cannot commit). Each removal is a distinct failure mode, testable independently.

- **Harsanyi dividend prediction:** In a running Flock, the distribution of Harsanyi dividends across Ask-sibling coalitions should be sparse - most coalitions should show near-zero dividends (redundant), with a small number of high-synergy coalitions carrying the decision load. This matches Qu et al.'s finding in LLM attention heads and is a direct prediction about ANY system built on this architecture.

- **Topological bound prediction:** Increasing the Ask-sibling coalition beyond six to seven topological neighbours should NOT improve decision quality and SHOULD increase energy cost. There is a thermodynamic cliff: below seven, cooperation degrades; above seven, energy cost increases without quality gain. This is testable in artificial Flock simulations.

- **GT-FEP correspondence:** The trajectory of a settling Flock vote should converge to a distribution indistinguishable from the Nash equilibrium of the equivalent stochastic game (as computed by GT-FEP). If the trajectories diverge, either the architecture is wrong or GT-FEP is wrong. Both are falsifiable.
